...
/*   Updated: 2024/06/27 13:59:12 by ahadj-ar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_input(int keysym, t_long *so_long)
{
	...
	if (!i)
		i = 1;
	so_long->str = ft_itoa(i);
	if (keysym == XK_w || keysym == XK_s || keysym == XK_a || keysym == XK_d)
	{
		ft_which_move(so_long, keysym);
		mlx_string_put(so_long->mlx, so_long->mlx_win, 0, 300,
			16777215, so_long->str);
...
void	ft_init_xpm(t_long *so_long, int *width, int *height)
{
	...
	if (so_long->img_wall == NULL || so_long->img_item == NULL
		...
		|| so_long->img_pleft == NULL || so_long->img_player_right == NULL)
